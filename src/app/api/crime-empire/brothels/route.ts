...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === "resolve_event") {
      ...
      // Synthetic choices generated by the RaidEscape minigame outcome — bypass the choices[] match.
      const isSyntheticRaidChoice = choice === "police_risk_escaped" || choice === "police_risk_arrested";
      const chosen = isSyntheticRaidChoice
        ? { label: "", action: choice, reward_cash: 0, reward_xp: 0 }
        : choices?.find((c) => c.action === choice);
