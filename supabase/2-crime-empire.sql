...
create table player_crime_experience (
  id uuid primary key default gen_random_uuid(),
...
create table contract_attempts (
  id uuid primary key default gen_random_uuid(),
  player_id uuid not null references crime_players(id) on delete cascade,
...
-- Simplified policies for player tables (add proper checks in production)
