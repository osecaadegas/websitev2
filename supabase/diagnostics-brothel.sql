...
SELECT
  ...
  pb.last_collection,
  ROUND(
    EXTRACT(EPOCH FROM (NOW() - COALESCE(pb.last_collection, pb.purchased_at))) / 3600.0,
    2
  )                                         AS hours_since_last_collect,
  ROUND(
    SUM(bw.income_per_hour)
    * (0.7 + pb.supply_drinks  / 100.0 * 0.3)
    * (0.7 + pb.supply_hygiene / 100.0 * 0.3)
...
GROUP BY pb.id, cp.username, bt.name, pb.supply_drinks, pb.supply_hygiene, pb.client_satisfaction, pb.last_collection, pb.purchased_at
ORDER BY cp.username;
